...
class SudokuImageProcessing:
    ...
    def get_warped(self):
        """Utilize a warp perspective transformation to extract the sudoku grid from the
    original image by identifying the corners of the grid and converting it into a standalone image.
        """
        ...
        """
        The following steps are employed to estimate the corner coordinates of
        the puzzle for the purpose of applying the necessary transformation.
        """
    ...
    def get_matrix(self):
        ...
        for i in range(self.game_size):
            ...
            for j in range(self.game_size):
                ...
                """
               Sanity Check: This process verifies that an unsolvable puzzle is not processed. It checks for instances
                where two cells in a row, column, or subgrid share the same value. When such a situation occurs, the cell 
                with the higher prediction score for the specified label takes the value of the element, while the other
                cell selects the value with the second-highest prediction score.
                """
                ...
                # map the image pixel values between 0-1# reshape the numpy array# convert the image to tensor
                # get the model prediction
    ...
    def plot_on_image(image, matrix, matrix_sol, game_dim):
        ...
        try:
            ...
            unwarped_mask = cv2.warpPerspective(template,np.linalg.inv(transform_matrix),(img_width, img_height))
